...
Screen.prototype._init = function() {
	...
	self._sendCommand(0x80);                                  // the suggested ratio 0x80
	self._sendCommand(SSD1306_SETMULTIPLEX);                  // 0xA8
	self._sendCommand(0x3F);
	self._sendCommand(SSD1306_SETDISPLAYOFFSET);              // 0xD3
	self._sendCommand(0x0);                                   // no offset
	self._sendCommand(SSD1306_SETSTARTLINE | 0x0);            // line #0
	self._sendCommand(SSD1306_CHARGEPUMP);                    // 0x8D
	self._sendCommand(0x14);
	self._sendCommand(SSD1306_MEMORYMODE);                    // 0x20
	self._sendCommand(0x00);                                  // 0x0 act like ks0108
	self._sendCommand(SSD1306_SEGREMAP | 0x1);
	self._sendCommand(SSD1306_COMSCANDEC);
	self._sendCommand(SSD1306_SETCOMPINS);                    // 0xDA
	self._sendCommand(0x12);
	self._sendCommand(SSD1306_SETCONTRAST);                   // 0x81
	self._sendCommand(0x9F); 
	self._sendCommand(SSD1306_SETPRECHARGE);                  // 0xd9
	self._sendCommand(0xF1);
	self._sendCommand(SSD1306_SETVCOMDETECT);                 // 0xDB
...
Screen.prototype._sendData = function(data) {
	 var self = this;
	 self._sendCommand(0x21); // columns
	 self._sendCommand(0);
	 self._sendCommand(127);
	 self._sendCommand(0x22); // rows
	 self._sendCommand(0);
	 self._sendCommand(7);
